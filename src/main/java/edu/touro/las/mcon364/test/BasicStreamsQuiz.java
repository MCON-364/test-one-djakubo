...
public class BasicStreamsQuiz {
    ...
    public double averagePassingScore() {
        return scoresByCourse.values().stream().filter(integers -> integers.stream().anyMatch(y -> y >= 70))
                .flatMap(List::stream).mapToDouble(Integer::doubleValue).average().orElse(0.0);
    }
}
